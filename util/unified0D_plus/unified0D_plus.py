...
def junction_loss_coeff(U, A, theta):
    ...
    pseudo_direction = np.sign(np.average(np.multiply(flow_rate[inlets], np.sin(theta[inlets]))))
    ...
    # total_pseudo_area = np.multiply(np.divide(flow_rate_total,
    ...
    C[outlets] = np.multiply(\
            (1-np.exp(-flow_ratio/0.02)),\
            np.subtract(1, np.multiply(
                np.reciprocal(np.multiply(area_ratio, flow_ratio)),\
                np.cos(0.75*(np.subtract(np.pi, phi)))))) # compute C
    K = np.NaN
