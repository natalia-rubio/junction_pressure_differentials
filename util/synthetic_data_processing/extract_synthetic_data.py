...
def collect_synthetic_results_steady(anatomy, require4 = True):
    ...
    geos = os.listdir(home_dir + f"/Desktop/synthetic_junctions_reduced_results/{anatomy}"); geos.sort()

    for j, geo in enumerate(geos[0:]):

        try:
            junction_params = load_dict(f"/home/nrubio/Desktop/synthetic_junctions/{anatomy}/{geo}/junction_params_dict")
            results_dir = f"/home/nrubio/Desktop/synthetic_junctions_reduced_results/{anatomy}/{geo}/"
            ...
            print(f"Problem extracting junnction data.  Skipping {geo}.")
            continue
