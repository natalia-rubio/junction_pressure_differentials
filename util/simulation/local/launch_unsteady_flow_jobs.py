...
def write_geo_job_unsteady(anatomy, geo_name):
    flow_name = "unsteady"
    geo = geo_name
...
/home/groups/amarsden/svSolver-github/BuildWithMake/Bin/svpost.exe -vtkcombo -start 0 -stop 100 -incr 1 -indir $indir -outdir $outdir -vtu solution_{flow_name}.vtu > /dev/null\n\
...
if __name__ == "__main__":
    ...
    while num_launched < num_geos:
        ...
        already_done = False
        if os.path.exists(f"/scratch/users/nrubio/synthetic_junctions_reduced_results/{anatomy}/{geo}/unsteady_red_sol"):
            already_done = False
        if already_done: print("Unsteady simulation already complete"); continue
        ...
        try:
            ...
            num_time_steps = 100
            write_svpre_unsteady(anatomy, geo_name, flow_params, copy.deepcopy(cap_numbers), inlet_cap_number, num_time_steps)
            write_unsteady_inp(anatomy, geo_name, flow_params, copy.deepcopy(cap_numbers), inlet_cap_number, num_time_steps)
            write_unsteady_flow(anatomy, geo_name, flow_params["flow_amp"], inlet_cap_number, num_time_steps)
            ...
            write_geo_job_unsteady(anatomy, geo)
