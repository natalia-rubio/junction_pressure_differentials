...
anatomy = sys.argv[1]; num_time_steps = int(sys.argv[2]); num_cores = int(sys.argv[3]); num_geos = int(sys.argv[4]); num_flows = int(sys.argv[5])

num_launched = 0
print(f"Launching {num_geos} steady flow sweeps.")
#anatomy = "Aorta_rand"#"mynard_over9"#"Aorta_u_40-60_over15"
...
while num_launched < num_geos:
    ...
    for i, inlet_flow_fac in enumerate([0.3, 0.6, 0.9, 1.2]):
        ...
        try:
            ...
            write_geo_job(anatomy, geo, flow_name = flow_name, flow_index = flow_index, num_cores = num_cores, num_time_steps = num_time_steps)
