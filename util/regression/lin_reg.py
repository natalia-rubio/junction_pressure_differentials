...
def train_lin_reg_model_steady(anatomy, num_geos, seed = 0, hyperparams = {}):
    ...
    pickle.dump(reg, open(f"/home/nrubio/Desktop/junction_pressure_differentials/results/models/{len(train_dataset)+len(val_dataset)}_lin_reg", 'wb'))
...
def train_lin_reg_model_unsteady(anatomy, num_geos, seed = 0, hyperparams = {}):
...
def train_lin_reg_model_unsteady_UO(anatomy, num_geos, seed = 0, hyperparams = {}):
    ...
    reg = LinearRegression().fit(np.asarray(train_input), np.asarray(train_output_UO))
    pickle.dump(reg, open(f"/home/nrubio/Desktop/junction_pressure_differentials/results/models/{len(train_dataset)+len(val_dataset)}_lin_reg", 'wb'))
