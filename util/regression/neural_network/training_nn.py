from util.gnn_util.training_util import *
...
def train_gnn_model(gnn_model, train_dataset, validation_dataset, train_params, network_params, dataset_params,
    ...
    dataset_name = f"{dataset_params['source']}_{dataset_params['features']}_{dataset_params['output']}_{dataset_params['filter']}"
    ...
    quad_loss = tf.math.sqrt(gnn_model.get_quad_loss(validation_output_tensor_data_loader,
        ...
        mse, scaling_dict))
    if model_name == None:
        model_name = str(network_params["hl_mlp"])[0:4] + "_hl_" + str(network_params["latent_size_mlp"])[0:4] + "_lsmlp_" + (str(train_params["learning_rate"])[0:6]).replace(".", "_") + "_lr_"+ "_bs_" + str(train_params["batch_size"]) + "_nepochs_" + str(train_params["nepochs"]) + dataset_name
