...
class NeuralNet():
    ...
    def update(self, indices):
        grads = grad(loss, argnums = [-4, -3, -2])(self.graph, #(self.data_dict["geo"][indices,:],
            self.scaling_dict,
...
def loss(graph, scaling_dict, encoder_weights, update_weights, decoder_weights, num_message_passing_steps):
    flow = graph.globals[3:8]; dP_true = graph.globals[8:13]
...
def predict(graph, encoder_weights, passing_weights, decoder_weights, num_message_passing_steps):
    ...
    nodes = batched_forward_pass(nodes, encoder_weights)

    for passing_step in range(num_message_passing_steps):
        sent_attributes = tree.tree_map(lambda n: n[senders], nodes)
        received_attributes = tree.tree_map(lambda n: n[receivers], nodes)

        updated_nodes = batched_forward_pass(jnp.concatenate([nodes[1:, :], received_attributes],axis = 1), passing_weights)
        nodes.at[1:].set(updated_nodes)

    nodes = batched_forward_pass(nodes, decoder_weights)

    return jnp.reshape(jnp.sum(nodes, axis=0), (-1, 3))