...
def train_svr_model_unsteady_UO(anatomy, num_geos, seed = 0, hyperparams = {}):
    ...
    svr0 = SVR(C=hyperparams["C"], epsilon = hyperparams["epsilon"]).fit(np.asarray(train_input), np.asarray(train_output_UO[:,0]))
    svr1 = SVR(C=hyperparams["C"], epsilon = hyperparams["epsilon"]).fit(np.asarray(train_input), np.asarray(train_output_UO[:,1]))
    svr2 = SVR(C=hyperparams["C"], epsilon = hyperparams["epsilon"]).fit(np.asarray(train_input), np.asarray(train_output_UO[:,2]))
