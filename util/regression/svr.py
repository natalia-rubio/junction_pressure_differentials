...
def train_svr_model_steady(anatomy, num_geos, seed = 0):
    ...
    svr0 = SVR(C=1.0, epsilon=0.2).fit(np.asarray(train_input), np.asarray(train_output[:,0]))
    svr1 = SVR(C=1.0, epsilon=0.2).fit(np.asarray(train_input), np.asarray(train_output[:,1]))
    pickle.dump(svr0, open(f"results/models/{len(train_dataset)+len(val_dataset)}_lin_svr0", 'wb'))
    pickle.dump(svr1, open(f"results/models/{len(train_dataset)+len(val_dataset)}_lin_svr1", 'wb'))
...
def train_svr_model_unsteady(anatomy, num_geos, seed = 0):
    ...
    svr0 = SVR(C=1.0, epsilon=0.2).fit(np.asarray(train_input), np.asarray(train_output[:,0]))
    svr1 = SVR(C=1.0, epsilon=0.2).fit(np.asarray(train_input), np.asarray(train_output[:,1]))
    svr2 = SVR(C=1.0, epsilon=0.2).fit(np.asarray(train_input), np.asarray(train_output[:,2]))

    pickle.dump(svr0, open(f"results/models/{len(train_dataset)+len(val_dataset)}_lin_svr0", 'wb'))
    pickle.dump(svr1, open(f"results/models/{len(train_dataset)+len(val_dataset)}_lin_svr1", 'wb'))
    pickle.dump(svr2, open(f"results/models/{len(train_dataset)+len(val_dataset)}_lin_svr2", 'wb'))
