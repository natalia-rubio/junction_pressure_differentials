...
def train_gpr_model_steady(anatomy, num_geos, seed = 0, hyperparams = {}):
    ...
    pred_coefs_val = tf.convert_to_tensor(gpr.predict(np.asarray(val_input)), dtype =tf.float64)
...
def train_gpr_model_unsteady_UO(anatomy, num_geos, seed = 0, hyperparams = {}):
    ...
    kernel = RBF(length_scale=hyperparams["length_scale"])
    gpr = GaussianProcessRegressor(kernel=kernel, random_state=0, alpha =hyperparams["alpha"]).fit(np.asarray(train_input), np.asarray(train_output_UO))
    pickle.dump(gpr, open(f"/home/nrubio/Desktop/junction_pressure_differentials/results/models/{len(train_dataset)+len(val_dataset)}_gpr", 'wb'))
